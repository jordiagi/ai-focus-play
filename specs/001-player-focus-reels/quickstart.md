...
- Before remote validation, confirm `ffmpeg` and `ffprobe` are installed or available in the remote `PATH`; the remote worker and export flow require both.
- On `ai-cluster`, the configured default is `AI_FOCUS_REMOTE_PATH_PREFIX=$HOME/ai-remote/bin:$HOME/ai-remote/.venv/bin`, where the project-owned remote venv and FFmpeg wrappers live.

## Implementation Validation Notes

- Backend syntax can be checked with `python -m compileall backend/src backend/tests`.
- Backend tests assume `PYTHONPATH=backend` so imports resolve from `backend/src`.
- Frontend tests are scaffolded for `vitest` and require `npm install` before execution.
